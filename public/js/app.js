function addProduct(productDetails, menuID){
    ...
    div.innerHTML='<div class="productImg"><a href="'+productDetails.img+'"><img src="images/'+productDetails.img+'"></a></div><div class="productBottom"><div class="productDetails"><h1 class="productName" title="'+productDetails.name+'">'+productDetails.name+'</h1><span class="price">'+productDetails.price+'</span></div><button class="buy">أضف إلي السلة</button></div>';
...
function prepareStore(){
    fetch('/get-all-products').then(response => response.json()).then(data => {productsMenus = data;addProductsMenus();addProducts()}).catch(error => console.error(error));
}
